...
pub const Client = struct {
    primary_curl_easy_handle: *curl.CURL,
    ...
    pub fn init(allocator: mem.Allocator, hostname: []const u8, encryption: Encryption) !Client {
        ...
        var client = Client{
            .primary_curl_easy_handle = undefined,
        ...
        client.primary_curl_easy_handle = try client.getCurlEasyHandle();
    ...
    /// Adds a request to the queue to be batched and returns a request index to it.
    pub fn enqueueRequest(self: *Client, allocator: mem.Allocator, method: std.http.Method, path: []const u8, body: ?[]const u8) !usize { //usize { // !Response {
        ...
        const request_index = self.curl_easy_handles.items.len;
        try self.curl_easy_handles.append(curl_easy_handle);
        return request_index;
    }

    fn awaitRequest(self: *Client, allocator: mem.Allocator, method: std.http.Method, path: []const u8, body: ?[]const u8) !Response {
        const curl_easy_handle = try self.getCurlEasyHandle();
        ...
        if (@enumToInt(status) == 0)
            // The request succeeded so `status` is guaranteed to be non-zero
            unreachable;
    ...
    /// Returns a response belonging to a request index, or `null` if the request hasn't been processed.
    pub fn getResponse(self: Client, request_index: usize) ?Response {
        var status: std.http.Status = undefined;
        if (curl.curl_easy_getinfo(self.curl_easy_handles.items[request_index], curl.CURLINFO_RESPONSE_CODE, &status) != curl.CURLE_OK)
            unreachable;
        if (@enumToInt(status) == 0)
            return null;
        log.debug("response status: {}", .{status});
    ...
    pub fn awaitAllRequests(self: Client) void {
        var polls: usize = 0;
...
test "parallel GET requests" {
    ...
    var request_indices = [_]usize{
        try client.enqueueRequest(testing.allocator, .GET, "/", null),
        try client.enqueueRequest(testing.allocator, .GET, "/", null),
        try client.enqueueRequest(testing.allocator, .GET, "/", null),
        try client.enqueueRequest(testing.allocator, .GET, "/", null),
        try client.enqueueRequest(testing.allocator, .GET, "/", null),
    ...
    client.awaitAllRequests();

    for (request_indices) |request_index| {
        const response = client.getResponse(request_index).?;
...
test "POST requests" {
    ...
    const response = try client.awaitRequest(testing.allocator, .POST, "/", "hello");
